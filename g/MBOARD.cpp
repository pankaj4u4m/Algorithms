...
int main ( int argc, char *argv[] ) {
	...
	REP(i, q){
		scanf("%s", name);
		if(strcmp(name, "RowSet") == 0){
			scanf("%d%d", &p, &x);
			row[p] = x>0?n:0;
			FOR(j, 1, n+1)col[j]+=x>0?1:0;
		} else if(strcmp(name, "ColSet") == 0){
			scanf("%d%d", &p, &x);
			col[p] = x>0?n:0;
			FOR(j, 1, n+1)row[j]+=x>0?1:0;
		} else if(strcmp(name, "RowQuery") == 0){
			scanf("%d", &p);
			printf("%d\n", n-row[p]);
		} else if(strcmp(name, "ColQuery") == 0){
			scanf("%d", &p);
			printf("%d\n", n-col[p]);
		}  
	}
