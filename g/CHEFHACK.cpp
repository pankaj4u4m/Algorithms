...
class BitSet{
	...
	public:
		BitSet(int n){
			int size = (n+1)/intSize;
			flag = (int*)calloc(size, intSize);
		}
		void setBit(int i){
			int x = i/intSize;
			int m = i%intSize;
...
int a[400][400];
int n;
void inValidate(int i, int j, int isEvent) {
	queue<pair<int, int> > q;
	q.push(MP(i, j));
	a[i][j] = -1;
	while(!q.empty()){
		pair<int, int> top = q.front();q.pop();
		if(top.first + 1 < n && a[top.first+1][top.second] != -1 && getPrimeCount(a[top.first+1][top.second]) == -1 && a[top.first+1][top.second]%2 != isEvent){
			a[top.first+1][top.second] = -1;
			q.push(MP(top.first+1, top.second));
		}
		if(top.second + 1 < n && a[top.first][top.second+1] != -1&& getPrimeCount(a[top.first][top.second+1]) == -1 && a[top.first][top.second+1]%2 != isEvent){
			a[top.first][top.second+1] = -1;
			q.push(MP(top.first, top.second+1));
		}
		if(top.first - 1 >=0 && a[top.first-1][top.second] != -1&& getPrimeCount(a[top.first-1][top.second]) == -1 && a[top.first-1][top.second]%2 != isEvent){
			a[top.first-1][top.second] = -1;
			q.push(MP(top.first-1, top.second));
		}
		if(top.second - 1 >= 0 && a[top.first][top.second-1] != -1&& getPrimeCount(a[top.first][top.second-1]) == -1 && a[top.first][top.second-1]%2 != isEvent){
			a[top.first][top.second-1] = -1;
			q.push(MP(top.first, top.second-1));
		}
	}
...
//	freopen("CHEFHACK.cppin","r",stdin);
	generatePrime(10000000);
//	REP(i, 50)cout<<prime[i]<< " ";
	INPUT;
	int T;//scanf("%d", &T);
	getInt(T);
	while(T-->0){
		...
		REP(i, n)REP(j, n){
			if(a[i][j] == -1){
				continue;
			} else if((x = getPrimeCount(a[i][j]) )!= -1){
	//			out(x)out(a[i][j]);
				res += x;
			} else if(a[i][j]%2 == 0){
				res += a[i][j]/2;
