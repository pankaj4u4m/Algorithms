...
//unsigned long long int mod = 10+7;

int main ( int argc, char *argv[] ) {
//	freopen("THREEDIF.cppin","r",stdin);
	int T;scanf("%d", &T);
	while(T-->0){
		unsigned long long int n[3];
		cin>>n[0]>>n[1]>>n[2];
		unsigned long long int ans = ((n[0]%mod)*(n[1]%mod))%mod;
		ans = (ans*(n[2]%mod))%mod;

		sort(n, n+3);

		ans -= ((n[0]%mod)*((n[2]-1)%mod))%mod;
		ans -= ((n[0]%mod)*((n[1]-1)%mod))%mod;
		ans -= ((n[0]%mod)*((n[0]-1)%mod))%mod;
		ans -= ((n[1] - n[0])%mod*(n[0]%mod))%mod;
		ans -= n[0]%mod;
		ans+=10*mod;
		cout<<(ans%mod)<<endl;
	}
