...
/**************** Main program *******************/
int A[100005];
int mintree[100005*100];
int maxtree[100005*100];

void preprocess(int p, int x, int y){
	if(y<x){
		return;
	}
	if(y == x){
		...
		preprocess(p*2, x, (x+y)/2);
		preprocess(p*2+1, (x+y)/2+1, y);
		mintree[p] = min(mintree[p*2], mintree[p*2+1]);
		maxtree[p] = max(maxtree[p*2], maxtree[p*2+1]);
	}
}

pair<int, int> query(int p, int x, int y, int i, int j){
	//cout<<x<<" " <<y<<" " <<i<<" " <<j<<endl;
	if(y<i || x>j){
		return MP(0, INF);	
	} else if(x<=i && j<=y){
		return MP(maxtree[p], mintree[p]);
	} else {
		pair<int, int> t1 = query(p*2, x, y, i, (i+j)/2);
		pair<int, int> t2 = query(p*2+1, x, y, (i+j)/2+1, j);
...
//	freopen("seggen1.cppin","r",stdin);
//	freopen("seggen2.cppin","r",stdin);
	freopen("seggen3.cppin","r",stdin);
...
//		outA(maxtree, 10);
		...
		for(int i = 1; i<n ;++i){
			for(int j = 0;j +i < n; j+=1){
				pair<int, int> q = query(1, j, j+i, 0, n-1);
	//			out(q.first)out(q.second)
