...
long long int comb(int n, int r){
	...
	C[n][r] = (comb(n-1, r) + comb(n-1, r-1)) %mod;
	return C[n][r];
}
long long int B[2*400005];
long long int S[2*400005];
int main ( int argc, char *argv[] ) {
	...
	while(r-->0){
		...
		long long int all = comb(m+n, n);
		long long int sub = 0;
		for(int i =  1; i<= b; ++i){
			B[b-i] = comb(n-a+b-i, b-i);
			S[i] = comb(a-1+i, i);
			long long x = B[b-i]* S[i];
			sub= (sub+x)%mod;
		}
//		outA(B, b+1);
//		outA(S, b+1);
		long long int ans =(all - sub + mod) %mod; 
//		out(all);
//		out(sub)
		out(ans);
		//printf("%lld\n", ans);
