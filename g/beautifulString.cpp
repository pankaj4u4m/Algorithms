...
//	freopen("beautifulString.cppin","r",stdin);
	int T;scanf("%d", &T);
	int Case = 1;
	while(T--){
		...
		for(int i = 0; i< n; ++i){
			...
			if(input[i]>='a' && input[i]<='z'){
				freq[input[i]-'a'] = 
				MP(freq[input[i]-'a'].first+1, input[i]);
			}
		}
		sort(freq, freq+26, fun);
		map<char, int> value;
		for(int i = 0; i< 26; ++i){
			value[freq[i].second] = i+1;
		}
		int res = 0;
		for(int i = 0; i< n; ++i){
			if(input[i]>='a' && input[i]<='z'){
				res+=value[tolower(input[i])];
			}
		}
		printf("Case #%d: %d\n", Case++, res);
