...
//	freopen("DIRECTI.cppin","r",stdin);
	...
	while(T-->0){
		...
		for(int i = 0; i< n; ++i){
			scanf("%s%s", dir, road);
			gets(road);
			path.push_back(make_pair(dir, road));
		}
		bool left = false;
		printf("Begin on%s\n", path[n-1].second.c_str());
		if(strcmp("Left", path[n-1].first.c_str()) == 0){
			left = true;
		}
		for(int i = n-2; i>=0; --i){
			if(left){
			...
			if(strcmp("Left", path[i].first.c_str()) == 0){
				left = true;
			} else {
				left = false;
			}
