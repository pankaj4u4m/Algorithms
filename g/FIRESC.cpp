...
int dfs(int x, bool flag[], map<int, vector<int> > &relation){
	int ret = 1;
	FE(it, relation[x]){
		if(!flag[*it]){
			flag[*it] = true;
			ret += dfs(*it, flag, relation);
		}
	}
	return ret;
}
...
//	freopen("FIRESC.cppin","r",stdin);
	...
	while(T-->0){
		...
		bool flag[n+5];memset(flag, 0, sizeof flag);
		long long ways = 1;
		long long number = 0;
		FOR(i, 1, n+1){
			if(!flag[i]){
