...
//		c//out<<right<< " "<<i<<" " <<left<<" " << j<< " "<<endl;
		while(i <= right && (j < left || maxtree[i] <= maxtree[j])){
			rightCount += maxcount[i];
			//out(leftCount)
				//out(maxcount[i])
			cnt+=leftCount * maxcount[i];
			ans *= pow((long long int)maxtree[i], leftCount * maxcount[i], mod)	;	
			ans%=mod;
...
//		c//out<<right<< " "<<i<<" " <<left<<" " << j<< " "<<endl;
		...
		while(j>=left && (i> right || maxtree[j] <= maxtree[i])){
			leftCount += maxcount[j];
			//out(rightCount)
				//out(maxcount[j])
			cnt+=rightCount * maxcount[j];
			ans *= pow((long long int)maxtree[j], rightCount * maxcount[j], mod)	;	
			ans%=mod;
...
//		c//out<<right<< " "<<i<<" " <<left<<" " << j<< " "<<endl;
	...
	ans*= fun(a, index-1);ans%=mod;
	ans*= fun(index+1, b);ans%=mod;
	return ans;
}
...
//	while(T-->0){
		...
		long long int ans = fun(0, n-1)%mod;
//		out(cnt);
		printf("%d\n", (int)ans);
//	}
	return EXIT_SUCCESS;
