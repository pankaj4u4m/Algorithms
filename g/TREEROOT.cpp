...
vector<pair<int, int> >findsum(int s){
	vector<pair<int, int> > ids;
	FE(it, id){
		if(!visited[*it] && *it == s){
			ids.push_back(make_pair(*it, -1));
		}
	}
	for(int i = 0; i< n; ++i){
		if(visited[id[i]]) {
		...
		for(int j = i+1;j< n; ++j){
			if(!visited[id[j]] && id[i] + id[j] == s){
				ids.push_back(make_pair(id[i], id[j]));
			}
		}
	}
...
bool found(int p){
	if(sum[p] == 0){
		...
		vector<pair<int, int> > pairs = findsum(sum[p]);
		FE(it, pairs){
	//	cout<<it->first<<" " <<it->second<<endl;
			visited[it->first] = true;
			visited[it->second] = true;
			if(found(sum[it->first]) && (it->second == -1 || found(sum[it->second]))){
				return true;
			}
		}
		return false;
...
//	freopen("TREEROOT.cppin","r",stdin);
	int T;scanf("%d", &T);
	while(T-->0){
		scanf("%d", &n);
		int x, y;
