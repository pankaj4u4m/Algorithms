...
long long v[2005][4005];
long long fun(int i, int c){
	if(i>=n){
		return 0;
	}
	if(v[i][c]){
//	  cout<<i<< " "<<dp[i]<< " " <<c-1<<" "<< endl;
		return dp[i][c];
	}
	long long mn = INF;
	int coin = 0;
	int took = 0;
	for(int j = i; j< min(n, c); ++j){
		long long mx = 0;
		++coin;
		long long sum = 0;
		int taken = 0;
		for(int k = j+1; k< min(n, j + 2*coin + 1); ++k){
			sum+=a[k];
			++taken;
			mx = max(mx, sum+fun(k+1, k+1+2*taken));
		}
		int x = min(mn, mx); 
		if(mn > x){
			mn = x;
...
//	while(T-->0){
	...
	long long x = a[0] + fun(1, 3);
	long long y = a[0] + a[1] + fun(2, 6);
	cout<<max(x, y)<<endl;
//	}
	return EXIT_SUCCESS;
