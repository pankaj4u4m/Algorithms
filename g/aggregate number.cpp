...
bool findAggregate(int i, int len, int n){
	int ab = n/power(10, i);
	if(dp[i]) {
		return dp[i] == 2; 
	}
	for(int x = 1; x<len-i; ++x){
			int firstnum = reverse(ab%(power(10, x)));
			int remfirst = ab/(power(10, x));
		for (int y = 1; y< len-i; ++y){
			int secondnum = reverse(remfirst%(power(10, y)));
			int remsecond = remfirst/(power(10, y));
			for (int z = max(x, y); z< len-i; ++z){
				int thirdnum = reverse(remsecond%(power(10, z)));
//				out(firstnum);
//				out(secondnum);
//				out(thirdnum);
				if(firstnum + secondnum == thirdnum){
					if(i+x+y+z == len){
						dp[i] = 2;
						return true;
					} else if( findAggregate(i+x, len, n) ){
						dp[i] = 2;	
						return true;
					}
...
bool aggregate(int n) {
	...
	return findAggregate(0, len, d); 
}

int main ( int argc, char *argv[] ) {
	//freopen("aggregate number.cppin","r",stdin);
	freopen("aggregate number.cppout","w",stdout);
//	cout<< 112358<<"=" << aggregate(112358)<<endl;
	//cout<< 122436<<"=" << aggregate(122436)<<endl;
	cout<< 1299111210<<"=" << aggregate(1299111210)<<endl;
	//cout<< 112112224<<"=" << aggregate(112112224)<<endl;
