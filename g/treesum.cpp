...
long long treesum(int i, int k, vector<int>G[], int l, vector<bool> v){
	long long sum = pow(l, k);
	for(vector<int>::iterator it = G[i].begin(); it != G[i].end(); ++it){
		if(!v[*it]) {
			v[*it] = true;
			sum+=treesum(*it, k, G,  l+1, v);
			sum%=mod;
		}
	}
	return sum%mod;
