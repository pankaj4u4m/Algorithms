...
int n;
int m;
vector<int> find(int pend, int cstart){
	vector<int> ret;
	for(int p = 0; p<= pend; p+=m){
		int i = 0;
		for(; i< m; ++i){
			if(k1[i+cstart] != k2[p+i] && k2[p+i] != '?' && k1[i+cstart] != '?'){
				break;
			}
		}
		if(i == m){
...
bool solve(int pend, int cstart){
	//out(pstart)out(pend)out(cstart);
	if(cstart >= n){
		return true;
	}
	if(pend<0){
	...
	vector<int> point = find(pend, cstart);
	bool flag = false;
	FE(it, point){
		for(int i = 0; i< m; ++i){
			swap(k2[i+*it], k2[pend-m+i+1]);
		}
		if(solve(pend-m+1, cstart+m)){
			if(!flag){
				for(int i = 0; i< m; ++i){
					if(k1[cstart+i] =='?' && k2[i+*it] == '?'){ 
						com[i] = 'a';
					} else if(k1[cstart+i] == '?'){
						com[i] = k2[i+*it];
					} else{
