...
int maxCount(int r[], int s, int e){
	int mx = r[s];
	int c = 1;
	FOR(j, s+1, e){
		if(mx < r[j]){
...
bool check(int r[], int n, int k, int m){
	...
	//int mx = 0;
	FOR(i, 0, n-k+1){
		int c = maxCount(r, i, i+k);
		
/*	while(!d.empty() && i - d.front().second >= k){
			d.pop_front();
		}
	  while(!d.empty() && d.back().first < r[i]){
			d.pop_back();
		}
		
		d.push_back(MP(r[i], i));
		pair<int, int> mx = *d.begin();
		c=0;
		FE(it, d){
			if(mx.first == it->first){
				c++;
			} else {
				break;
			}
...
int main ( int argc, char *argv[] ) {
	freopen("LEALCO.cppin","r",stdin);
	int T;scanf("%d", &T);
	while(T-->0){
		int n, k, m;scanf("%d%d%d", &n, &k, &m);
		int r[n+1];REP(i, n)scanf("%d", &r[i]);
		int result = INF;
		bool f = false;
		int _n = (1<<n)+1;
		for(int i = 0;i<_n; ++i){
