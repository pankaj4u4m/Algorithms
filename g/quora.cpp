...
map<pair<double, double>, vector<int> > top;
map<int, vector<int> >que;

bool comp(pair<double, int> a, pair<double, int> b){
	if(abs(a.first - b.first) < 0.001){
		return a.second > b.second;
	} else return a.first < b.first;
}

vector<pair<double, int> > findT(double x, double y){
	vector<pair<double, int> > d;
	FE(it, top){
		double dis =  sqrt(sqr(it->first.first - x) + sqr(it->first.second - y));
		FE(c, it->second){
			d.push_back(MP(dis, *c));
		}
	}
...
vector<pair<double, int> > findQ(double x, double y){
	vector<pair<double, int> > d;
	FE(it, top){
		double dis =  sqrt(sqr(it->first.first - x) + sqr(it->first.second - y));
		FE(c, it->second){
			FE(q, que[*c]){
				d.push_back(MP(dis, *q));
			}
		}
	}
	sort(d.begin(), d.end(), comp);
...
int main ( int argc, char *argv[] ) {
	//freopen("quora.cppin","r",stdin);
	int T, Q, N;scanf("%d%d%d", &T, &Q, &N);
	int t;
	double x, y;
	REP(i, T){
		scanf("%d%lf%lf", &t, &x, &y);
		top[MP(x, y)].push_back(t);
	}
	int q, n;
	REP(i, Q){
		scanf("%d%d", &q, &n);
