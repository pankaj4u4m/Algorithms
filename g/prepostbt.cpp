...
Node * constructTree(int pre[], int post[], int *p, int posts, int poste, int n){
	if(*p >=n || posts > poste){
		return NULL;
	}
	Node *root = new Node();root->data = pre[*p];++*p;
	if(posts == poste){
		return root;
	}
	int i = posts;
	...
	root->left = constructTree(pre, post, p, posts, i, n);
	root->right = constructTree(pre, post, p, i+1, poste, n);
	return root;
}


int main ( int argc, char *argv[] ) {
	//freopen("prepostbt.cppin","r",stdin);
	int pre[] = {1, 2, 4, 8, 9, 5, 3, 6, 7};
  int post[] = {8, 9, 4, 5, 2, 6, 7, 3, 1};
  int size = sizeof( pre ) / sizeof( pre[0] );
 	int i = 0;
  struct Node *root = constructTree(pre, post, &i, 0, size -1, size);
	printInorder(root);	
	return EXIT_SUCCESS;
