...
long long vertical(int k){
	long long int v = 1, c=2;
	while(k){
		if(k&1){
			long long sum = 0;
			for(int i = 0;i<(1<<2); ++i){
				...
				for(int j = 0; j< 2; ++j){
					if(i&(1<<j)){
						...
						if(tiles.insert(j).second){
							++count;
						}
						if(j-1>=0 && tiles.insert(j-1).second){
							++count;
						}
						if(j+1<2 && tiles.insert(j+1).second){
							++count;
						}
					}
				}
				outC(tiles);
				long long x = count == 0? v : (v-1)/(count);
				long long y = count == 0? c : (c-1)/count;
				x=x*y;
				out(x);
				sum+=x;
			}
		...
		for(int i = 0;i<(1<<2); ++i){
			...
			for(int j = 0; j< 2; ++j){
				...
				outC(tiles);
			long long x = count == 0? c : (c-1)/count;
			sum+=(x == 0?1:x )*( x == 0?1:x);
		}
		c = sum;
		k>>=1;
...
int main ( int argc, char *argv[] ) {
	//freopen("PocketGem.cppin","r",stdin);
//	int T;
//	while(T-->0){
//		scanf("%d%d", &n, &k);
		//int v = vertical(n, k);
