...
#endif
/**************** Main program *******************/
int n, total, totalQuality = 0;
vector<pair<int, int> >graph[6];
int sz[6];

void travel(int i, int cost, int q){
	if(cost>total || q<= totalQuality) return;
	if(i == 6){
		totalQuality = q;
		return;
	}
	REP(j, sz[i]){
		travel(i+1, cost+graph[i][j].first, min(q, graph[i][j].second));
	}
}

int main ( int argc, char *argv[] ) {
	freopen("soldier.cppin","r",stdin);
	int type, price, quality;
	scanf("%d%d", &n, &total);
	REP(i, n){
		scanf("%d%d%d", &type, &price, &quality);
		graph[type-1].push_back(make_pair(price, quality));
	}
	REP(i, 6)sz[i]=graph[i].size();
	travel(0, 0, INF);
	printf("%d\n", totalQuality);
	return EXIT_SUCCESS;
