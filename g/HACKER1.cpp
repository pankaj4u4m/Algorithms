...
pair<LL, pair<LL, LL> > extgcd(long long a, long long b){
	if(b == 0){
		return MP(a, MP(1, 0));
	} else {
		pair<LL, pair<LL, LL> > p = extgcd(b, a%b);
		return MP(p.first, MP(p.second.second, p.second.first - a/b * p.second.second));
	}
}

long long divide(long long r, int d){
	pair<LL, pair<LL, LL> > p = extgcd(r, d);
	return p.second.first*d/p.first;
}

long long solve(int n, int k){
	long long x = max(n-k, k);
	long long y = min(n-k, k);
	long long t = n;
	long long ret = 1;
	int s = 2;
	while(t>x){
...
int main ( int argc, char *argv[] ) {
	freopen("HACKER1.cppin","r",stdin);
	int T;scanf("%d", &T);
	while(T-->0){
		int n, k;scanf("%d%d", &n, &k);
		int a[n+5];REP(i, n)scanf("%d", &a[i]);
		long long ans = 0;
		sort(a, a+n);
		FOR(i, 0, n-k+1){
			ans+= a[n-i-1] * solve(n-i-1, k-1);
			ans%=mod;
		}
		cout<<ans<<endl;
	}
