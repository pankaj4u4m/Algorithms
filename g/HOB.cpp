...
int rng(){
	...
	int n = strlen(name);
	for(int i = 0;i< n-1; ++i){
		ret += (tolower(name[i])-'a'+1)* 33;
		ret%=r;
	}
	ret += (tolower(name[n-1])-'a'+1);
	return (ret + waiting)%r;
}


int occupyRoom(int m, int g){
	int no = rng();
//	out(no);
	int a = 0, b=1, c=2;
	long long int minValue = INF;
	for(int i = 0; no/r <= 2; ++i){
		no+=a;
		minValue = min(minValue, room[no%r]);
//		out(no%r)
		if(room[no%r] <= timeStamp+m){
			room[no%r] = timeStamp+m+g*60;
			return i;
		} else {
			int t = a+b+c;
			a = b;
...
//			out(timeStamp);
			long long int x = occupyRoom(h *60 + m, g);
			cout<<x<<endl;
			waiting += x<0?-x:x;
			prev = h*60+m;
//			out(waiting);
