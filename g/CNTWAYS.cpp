...
//const long long int mod = 28;
long long int B[400005];
long long int S[400005];
long long int dp[400005];

pair<int, pair<long long int, long long int> >extend_gcd(long long int a, long long int b){
	if(b == 0){
		return MP(a, MP(1L, 0L));
	} else {
		pair<int, pair<long long int, long long int> >p = extend_gcd(b, a%b);
		return MP(p.first, MP(p.second.second, p.second.first - a/b * p.second.second));
	}
}

long long int fun(long long c, int d){
	if(!dp[d]){
		dp[d] = extend_gcd(mod, d).second.second;
	}
	return (dp[d] * c) %mod;
}

int main ( int argc, char *argv[] ) {
//	freopen("CNTWAYS.cppin","r",stdin);
//	freopen("rectgen.cppin","r",stdin);
	int r;scanf("%d", &r);
	while(r-->0){
		int m, n, a, b;
		scanf("%d%d%d%d", &n, &m, &a, &b);
		long long int all = 1;//comb(m, n);
		long long int sub = 0;
		B[0] = 1;
		S[0] = 1;
		for(int i = 1; i<=m; ++i){
