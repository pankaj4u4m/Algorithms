...
#endif
/**************** Main program *******************/
void rotate(char *c, int first, int middle, int end){
	int next = middle;
	while(first != next){
	...
	out(next)out(middle)out(first)
}

void interlieved(char *a, char *b, int n, int m, int i, int j, char *res){
	...
	if(i==n){
		...
		res[i+j] = a[i];
		interlieved(a, b, n, m, i+1, j, res);
		res[i+j] = b[j];
		interlieved(a, b, n, m, i, j+1, res);
	}
}

int main ( int argc, char *argv[] ) {
	//freopen("rotate.cppin","r",stdin);
	char c[] = "PankajKumar";
	rotate(c, 0,7, 11);
	out(c);
	char a[] = "AB";
