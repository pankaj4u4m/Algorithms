...
Node * construct(int **pre, int *n, int *in, int size){
	if(*n <0){
		return NULL;
	}
	Node *root = new Node();
	root->value = **pre;
	int x = lower_bound(in, in+size+1, **pre)-in;
	++*pre;
	--*n;
	if(x-1>=0){
		root->left = construct(pre, n, in, x-1);
	}
	if(x+1 <= size && size-x-1>=0){
		root->right = construct(pre, n, &in[x+1], size-x-1);
	}
	return root;
}
...
int main ( int argc, char *argv[] ) {
	//freopen("pocketgeminterview3.cppin","r",stdin);
	Node *root = new Node();root->value = 4;
	for(int i = 3; i< 15; ++i){
		insert(root, rand()%20);
	}
	printPreOrderRec(root);puts("");
	printPreOrder(root);puts("");
	printInOrderRec(root);puts("");
	printInOrder(root);puts("");
	printPostOrderRec(root);puts("");
	printPostOrder(root);puts("");

	int a[] = {4, 1, 0, 2, 7, 4, 4, 5, 5, 14, 9, 18, 18};
	int *pre = a;
	int in[13];
	int n = 13;
	for(int i =0; i<n; ++i){
	...
	sort(in, in+n);
	n-=1;
	Node *tree = construct(&pre, &n, in, n);
	printPreOrderRec(tree);puts("");
	printInOrderRec(tree);puts("");
	return EXIT_SUCCESS;
