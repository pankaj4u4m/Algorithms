...
void printMatch(char h[], char c[], int n, int m){
	precompile(c, m);
	outA(A, m);
	int k = 0;
	for(int i = 0; i< n; ++i){
...
int main ( int argc, char *argv[] ) {
	//freopen("kmp.cppin","r",stdin);
	;
	char needle[] = "kumar";
	char haystack[] = "pankajkumarxxkuxmarkumarxx";
	printMatch(haystack, needle, strlen(haystack), strlen(needle));
	return EXIT_SUCCESS;
