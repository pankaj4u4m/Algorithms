...
void inplaceRotate(int **a, int n){
...
void inplacerotate(){
	...
	REP(i, 4)outA(a[i], 4);puts("");
	inplaceRotate(a, 4);
	REP(i, 4)outA(a[i], 4);
}

int randNotInArray(int *a, int k, int n){
	int x = rand()%(n-k);
	out(x);
	if(a[k-1]-k<x){
		return x+k;
...
int cycledfs(int s, vector<int>G[], vector<int>&c, vector<bool>&v){

	FE(it, G[s]){
		if(!v[*it]){
			v[*it] = true;
			c[*it] =1+cycledfs(*it, G, c, v);
		}
		c[s] = max(c[s], c[*it]);
	}
	out(c[s]);
	return c[s]<0?-INF:c[s];
}
void cycleDetect(){
	int n = 10;
	vector<int>G[n+1];
	for(int i = 0; i< n; ++i){
		if(i<n)G[i].push_back((i+1)%n);
...
int madianTwoSortedArray(int *a, int *b, int n, int m){
	int k = (n-1+m-1)/2;
	if(n > m){
		swap(a, b);
		swap(n, m);
...
void madianTwoSortedArray(){
	int a[] = {1, 2, 3, 4, 5, 6, 768, 7};
	int b[] = {4, 5, 6, 7, 8, 9, 10, 234, 456};
	vector<int> v;
	for(int i = 0; i< 8; ++i){
		v.PB(a[i]);
	}
	...
	outC(v);
	cout<<v[9]<<endl;
	cout<<madianTwoSortedArray(a, b, 8, 9);
}

int main ( int argc, char *argv[] ) {
	//freopen("dummy.cppin","r",stdin);
	int *p = new int();
	*p++;
	main(2, NULL);
	return EXIT_SUCCESS;
